...
const RUSH_MULT  = 1.8;        // 分数倍率（底薪，爆发交给燃点）
    ...
    ['狂欢局　' + RUSH_EVERY + ' 局攒一次，今天打得多门槛会降', [
      ...
      { dot: '✦', name: '燃点', meta: '热度 >' + BURN_FROM,
        text: '热度过 ' + BURN_FROM + ' 之后倍率额外起飞，越高涨得越凶。'
              + '低于 ' + BURN_FROM + ' 两边完全一样 —— 前中期不白赚，打得够深才炸' },
...
// ── 燃点（只在狂欢局）──
// 狂欢局原来的全部优势就是一个恒定的 ×2.4 —— 那不叫爆发，叫底薪高：
// 整局每一分都均匀乘同一个数，方差一点没变。
// 现在改成：热度过 BURN_FROM 之后倍率额外起飞，低于它跟普通局完全一样。
// 门槛按实测热度轨迹定：狂欢局 100 行热度 106~128、200 行 152~162、
// 300 行约 230（热度确实随行数涨 —— 等级高、重力快、单位时间消行多）。
// 取 140 的效果（每行价值对比，含 RUSH_MULT，普通局含深局加成）：
//   100 行 1.00×  还点不着，前期两边完全一样 —— 狂欢局不白赚
//   200 行 0.93×  刚擦着点开，中期开始有感觉
//   300 行 1.42×  爆发
// 取 220 要 400 行才真开口，而狂欢局灰线快 82%、根本活不到那儿，机制白设；
// 取 160 要 250 行才动；取 120 则 100 行就点着，变回「底薪高」。
const BURN_FROM = 140;
const BURN_K = 1.2;
