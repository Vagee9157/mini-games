...
const RUSH_MULT  = 1.5;        // 分数倍率（低底薪，上限交给燃点）
  ...
  return [
    ...
    ['狂欢局　' + RUSH_EVERY + ' 局攒一次，今天打得多门槛会降', [
      ...
      { dot: '✦', name: '燃点', meta: '热度 >' + BURN_FROM,
        text: '热度过 ' + BURN_FROM + ' 之后倍率额外起飞，越高涨得越凶 —— 300 时 ×15、'
              + '445 时 ×27、600 时 ×42（普通局同热度只有 ×10.6 / ×12.2 / ×13.6）。'
              + '低于 ' + BURN_FROM + ' 两边完全一样' },
...
//   热度 220 以下 两边同值（打得一般的狂欢局不白赚）
//   热度 300 → ×15.2（普通局 ×10.6）   445 → ×27.2（普通局 ×12.2）
//   热度 600 → ×41.9                  800 → ×63.5
// 配合 RUSH_MULT 从 2.4 收到 1.5：低底薪 + 无上限，这才是「刺激」。
// 狂欢局本来就最容易堆热度（道具翻倍的额外注入 + 梭哈整条翻倍），
// 所以这个上限是它够得着的，不是画饼。
const BURN_FROM = 220;
const BURN_K = 1.2;
