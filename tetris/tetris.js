...
// 门槛按实测热度轨迹定：狂欢局 100 行热度约 110、200 行约 152、300 行约 230
// （热度确实随行数涨 —— 等级高、重力快、单位时间消行多）。
// 取 160 的效果：
//   100 / 200 行 还没点着，两边同值 —— 前中期狂欢局不白赚
//   300 行 ×14.6（普通局 ×9.0），分数 1.44 倍 —— 爆发只属于打得够深的局
// 取 220 就要 400 行才真开口，而狂欢局灰线快 82%、根本活不到那儿，
// 等于机制白设；取 130 会让 200 行就开，又变回「底薪高」。
// 配合 RUSH_MULT 从 2.4 收到 1.5：低底薪 + 无上限。
const BURN_FROM = 160;
const BURN_K = 1.2;
